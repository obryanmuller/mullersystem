...
// Funções utilitárias
const formatDate = (dateString: string) => new Date(dateString).toLocaleDateString('pt-BR', { dateStyle: 'short', timeStyle: 'short' });
const formatCurrency = (value: number) => `R$ ${value.toFixed(2).replace('.', ',')}`;
const todayISO = new Date().toISOString().split('T')[0];
const lastWeekISO = new Date(new Date().setDate(new Date().getDate() - 7)).toISOString().split('T')[0];
...
export default function CaixaPage() {
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            fetchData();
        }, 300); // Debounce leve para evitar chamadas excessivas ao mudar a data

        return () => clearTimeout(timer);
    }, [fetchData]);
    
    // Lógica para o Gráfico de Fluxo de Caixa
    const chartData = {
        labels: data?.grafico.map(d => new Date(d.data).toLocaleDateString('pt-BR', { month: 'short', day: 'numeric' })) || [],
        datasets: [
            {
                label: 'Entradas (R$)',
                data: data?.grafico.map(d => d.entradas) || [],
                backgroundColor: '#23a383', // Cor Verde
                stack: 'Stack 0',
            },
            {
                label: 'Saídas (R$)',
                data: data?.grafico.map(d => d.saidas * -1) || [], // Saídas como valores negativos para visualização
    ...
    const chartOptions = {
        ...
        scales: {
            ...
            y: {
                ...
                ticks: {
                    // Formata os valores negativos de volta para positivo no eixo Y
    ...
    const handleFormSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch('/api/caixa', {
                ...
                body: JSON.stringify({
                    tipo: formType,
                    // Garante que o valor seja enviado no formato correto (ponto decimal)
    ...
    // Função para gerar o CSV (apenas da tabela no período selecionado)
    const handleGenerateReport = () => {
        ...
        const csvRows = dataToExport.map(mov => {
            const valor = mov.valor.toFixed(2).replace('.', ','); // Valor com vírgula
        ...
        if (link.download !== undefined) {
            ...
            link.setAttribute("download", `relatorio_caixa_${data?.periodo.startDate}_a_${data?.periodo.endDate}.csv`);
            link.style.visibility = 'hidden';
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
        } else {
            alert('Seu navegador não suporta download automático. O relatório será exibido no console.');
    ...
    return (
        <>
            <div className="w-full">
                <h1 className="text-3xl font-bold text-brand-dark mb-6">Controle de Fluxo de Caixa</h1>
                ...
                <div className="bg-white p-6 rounded-lg shadow-md mb-6">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                        ...
                        <div className="flex gap-2">
                            <button 
                                onClick={() => { setFormType('ENTRADA'); setIsFormModalOpen(true); }}
                            ...
                            <button 
                                onClick={() => { setFormType('SAIDA'); setIsFormModalOpen(true); }}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mt-6 pt-4 border-t border-gray-100">
                         <div className="p-4 rounded-lg bg-green-50">
                            ...
                            <p className="text-xl font-bold text-green-900">{isLoading ? 'R$ 0,00' : formatCurrency(data?.kpis.totalEntradas || 0)}</p>
                        </div>
                        <div className="p-4 rounded-lg bg-red-50">
                            <p className="text-sm font-medium text-red-700">Total Saídas</p>
                            <p className="text-xl font-bold text-red-900">{isLoading ? 'R$ 0,00' : formatCurrency(data?.kpis.totalSaidas || 0)}</p>
                        </div>
                        <div className="p-4 rounded-lg bg-brand-light">
                            <p className="text-sm font-medium text-brand-dark">Saldo Final</p>
                            <p className={`text-xl font-bold ${data?.kpis.saldoFinal! >= 0 ? 'text-brand-green' : 'text-red-600'}`}>{isLoading ? 'R$ 0,00' : formatCurrency(data?.kpis.saldoFinal || 0)}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-md h-96">
                        <h2 className="text-xl font-semibold text-brand-dark mb-4">Fluxo de Caixa (Gráfico Dinâmico)</h2>
                        {isLoading ? (
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-md overflow-x-auto">
                        <h2 className="text-xl font-semibold text-brand-dark mb-4">Movimentações do Período</h2>
                            ...
                            <table className="min-w-full divide-y divide-gray-200 text-sm">
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {data?.movimentacoes.length === 0 ? (
                                        <tr><td colSpan={3} className="text-center py-4 text-gray-500">Nenhuma movimentação encontrada.</td></tr>
                                    ) : (
                                        data?.movimentacoes.map(mov => (
                                            <tr key={mov.id} className={mov.tipo === 'ENTRADA' ? 'bg-green-50' : 'bg-red-50'}>
                                                <td className="px-3 py-2 whitespace-nowrap">{formatDate(mov.dataHora)}</td>
                                                <td className="px-3 py-2">{mov.descricao}</td>
                                                <td className={`px-3 py-2 text-right whitespace-nowrap font-bold ${mov.tipo === 'ENTRADA' ? 'text-green-600' : 'text-red-600'}`}>
            ...
            {/* Modal de Registro de Movimentação (Entrada/Saída) */}
            {isFormModalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
                    <div className="bg-white p-8 rounded-lg shadow-2xl w-full max-w-md">
                        <h2 className={`text-2xl font-bold mb-6 ${formType === 'ENTRADA' ? 'text-brand-green' : 'text-red-600'}`}>
                            {formType === 'ENTRADA' ? 'Registrar Entrada' : 'Registrar Saída'}
                        </h2>
                        <form onSubmit={handleFormSubmit}>
                            ...
                            <div className="mb-6">
                                <label htmlFor="descricao" className="block text-sm font-medium text-gray-700">Descrição/Motivo</label>
                                <input 
                                    type="text"
                                    id="descricao"
                            ...
                            <div className="flex justify-end gap-4">
                                <button type="button" onClick={() => setIsFormModalOpen(false)} className="px-6 py-2 bg-gray-200 text-gray-800 font-semibold rounded-lg hover:bg-gray-300">
                                    Cancelar
                                </button>
                                <button type="submit" className={`px-6 py-2 font-bold text-white rounded-lg hover:opacity-90 ${formType === 'ENTRADA' ? 'bg-brand-green' : 'bg-red-600'}`}>
                                    Registrar
                                </button>
                            </div>
                        </form>
