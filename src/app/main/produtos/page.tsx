...
export default function ProdutosPage() {
  ...
  const handleConfirmDelete = () => {
    ...
    console.log("Excluindo produto:", selectedProduct.nome);
    alert(`Produto "${selectedProduct.nome}" excluído! (no console)`);
  ...
  return (
    <>
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {paginatedData.map((produto) => (
                  <tr key={produto.id} className={selectedItems.includes(produto.id) ? 'bg-green-50' : ''}>
                    <td className="px-6 py-4 align-middle"><input type="checkbox" checked={selectedItems.includes(produto.id)} onChange={() => handleSelectItem(produto.id)} className="h-4 w-4 rounded border-gray-300 text-brand-green focus:ring-brand-green" /></td>
                    <td className="px-6 py-4 align-middle"><div className="font-medium text-gray-900">{produto.nome}</div><div className="text-sm text-gray-500">{produto.sku} - R$ {produto.preco.toFixed(2)}</div></td>
                    ...
                    <td className="px-6 py-4 align-middle text-right text-sm font-medium">
                      <div className="flex items-center justify-end gap-4">
                        <button onClick={() => handleOpenEditModal(produto)} className="text-gray-400 hover:text-blue-600" title="Editar"><ActionIcon path="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></button>
                        <button onClick={() => handleOpenDeleteModal(produto)} className="text-gray-400 hover:text-red-600" title="Excluir"><ActionIcon path="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-4v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      <ModalAdicionarProduto isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} />
      <ModalEditarProduto isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} produto={selectedProduct} />
      <ModalExcluirProduto isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={handleConfirmDelete} produto={selectedProduct} />
    </>
  );
}